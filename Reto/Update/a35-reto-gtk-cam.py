...
VIDEO_DEVICE = "/dev/video7" # Update this to your webcam interface!
...
class SensorDashboard(Gtk.Window):
    def __init__(self):
        ...
        # GTK Image widget that will hold the OpenCV frames
        ...
        if not self.cap.isOpened():
            ...
            # Add a 33ms timeout (~30 FPS) to constantly update the camera frame
    ...
    def setup_css(self):
        ...
        .title { font-size: 26px; font-weight: 800; color: #00e5ff; }
    ...
    def update_camera_frame(self):
        ...
        if ret:
            # OpenCV uses BGR natively, GTK needs RGB
            frame_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            h, w, d = frame_rgb.shape
            
            # Convert raw bytes into a GTK Pixbuf
        ...
        return True # Returning True keeps the 30FPS loop running
    ...
    def process_buffer(self):
        while '\n' in self.buffer:
            ...
            if line.startswith("[NEO6MV2]:"):
                self.lbl_gps.set_text(line.replace("[NEO6MV2]:", "").strip())
